...
import { commentsForVersion } from "./media-version-comments";
...
describe("Media Version Comments seam", () => {
  it("keeps comments tied to their Media Version", () => {
    expect(
      commentsForVersion(comments, "v1").map((comment) => comment.id)
    ).toEqual(["old"]);
  });

  it("parses only comment rows from a transport response", () => {
    expect(
      parseMediaVersionComments({
        comments: [...comments, { body: "missing id" }],
      })
    ).toEqual(comments);
  });
